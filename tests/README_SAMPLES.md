...
it('should fetch cloned sample data and populate form', async () => {
  ...
  wrapper.vm.$axios.get = vi.fn().mockResolvedValueOnce({ data: clonedSampleData });
...
it('should return error when sample name is too short', () => {
  ...
  expect(wrapper.vm.validationErrors.name).toBe('Name must be between 3 and 80 characters.');
});
```
